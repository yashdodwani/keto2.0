    ...
    const fetchData = async () => {
      try {
        ...
        if (!transcriptResponse.ok) {
          throw new Error(transcriptData.error || 'Failed to fetch transcript. Please ensure the video has captions enabled.');
        }

        console.log(`Transcript fetched successfully using method ${transcriptData.method || 'unknown'}`);
        ...
        if (!analyzeResponse.ok) {
          throw new Error(analyzeData.error || 'Failed to analyze transcript');
        }
              ...
              {courseData.sections.map((section, index) => (
                ...
                >
                  ...
                  <div className="flex items-center text-xs text-gray-500 space-x-2">
                    ...
                    <span>
                      {Math.floor(section.start_second / 60)}:{Math.floor(section.start_second % 60).toString().padStart(2, '0')} - {Math.floor(section.end_second / 60)}:{Math.floor(section.end_second % 60).toString().padStart(2, '0')}
                    </span>
                  </div>
                  <p className="text-xs text-gray-600 mt-1 line-clamp-2">{section.summary}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    Go to {Math.floor(currentSection.start_second / 60)}:{Math.floor(currentSection.start_second % 60).toString().padStart(2, '0')}
                  </Button>
                </div>
                <CardDescription>
                  Section {currentSectionIndex + 1} of {courseData.sections.length}
                  {' • '}
                  Duration: {Math.floor((currentSection.end_second - currentSection.start_second) / 60)}:{Math.floor((currentSection.end_second - currentSection.start_second) % 60).toString().padStart(2, '0')}
                </CardDescription>
              </CardHeader>
              <CardContent>
